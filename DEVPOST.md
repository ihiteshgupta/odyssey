...
This is the first screen. Every number below is asserted today by the cited file/test — the deterministic safety eval runs fully offline, in CI.
...
| ADK LLM eval | Stale expected-trajectory + brittle ROUGE → **flaky/failing** (a live run scored trajectory **0.0**) | **Regenerated from real captured runs + semantic `final_response_match_v2` judge → reliably green (4/4 live)** | `evals/odyssey.evalset.json`, `evals/test_config.json` |
...
| Guardrail block-rate (unsafe checkouts denied) | — | **100% (6/6)** unsafe calls denied | `tests/test_safety_eval.py` |
| False bookings (within-budget mismatch + empty cart) | — | **0** | `tests/test_safety_eval.py` |
...
- **SAM** — APAC corporate travel **~$700B in 2026**, the world's fastest-growing region (**+10.9% YoY**, GBTA). The governance gap is documented: corporate booking compliance is only **~42%** (Deloitte 2025) → **~58% of bookings go out-of-policy**, and **~65% of business travel is unmanaged** because SMBs are priced out of Concur/Navan's ~$15–54K/yr floors.
...
**Autonomy is table stakes — enforced governance is the wedge.** Navan's *Ava*, SAP Concur's *Joule*, and Spotnana's *Otto* (live since Dec 2025) all book trips today — so Odyssey does **not** compete on "an AI that books a trip." It competes on **provably governed spend**. AP2 *describes* budget and policy in its signed mandates but **explicitly leaves enforcement to the app**; Odyssey is that enforcement layer — a deny-by-default budget guardrail (fails closed) + a non-skippable human-approval gate + a signed mandate-chain audit trail, scoped to corporate travel. Even Otto, the closest comparable, books autonomously but ships **no** approval/audit governance. *That gap is the product.* **Why now:** AP2 (Sept 2025, 60+ partners incl. Mastercard/Amex/PayPal), UCP, and A2A (Linux Foundation v1.0, 150+ orgs) all just landed — building the enforcement layer natively on Google's own stack, first, in corporate travel is the timing edge (forward-leaning plumbing, not the moat itself).

**The trust layer IS the product.** The buyer isn't shopping for a travel chatbot — they want a **governance artefact**: a record that every booking was pre-approved against a budget the agent *could not* exceed, with a signed mandate-chain audit trail (AP2) finance can pull on demand. The deny-by-default guardrail + non-skippable HITL + signed AP2 mandate chain are first-class components, not afterthoughts — the insurance policy that makes a CFO comfortable letting AI touch the card. **Friction is the feature.** This is what makes autonomous travel procurement/finance *actually approve*.
